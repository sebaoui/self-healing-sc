...
![Self-Healing Web Scrapers](IMAGE_PLACEHOLDER_HERO)

> **Note:** One of our community members wrote this guide as a contribution to the Crawlee Blog. If you'd like to contribute articles like these, please reach out to us on Apify's [Discord channel](https://apify.com/discord).
...
1. [The original scraper](#step-1-the-original-scraper)
2. [Preparing the test environment](#step-2-preparing-the-test-environment)
3. [The JSON bridge](#step-3-the-json-bridge)
4. [The self-healing scraper](#step-4-the-self-healing-scraper)
5. [Breaking and healing — the real test](#the-real-test-breaking-and-healing)
6. [Limitations & trade-offs](#limitations--trade-offs)
7. [Scraping Cloudflare-protected sites](#going-further-scraping-cloudflare-protected-sites)
8. [Deploying as an Apify Actor](#deploying-as-an-apify-actor)
...
* [Crawlee for Python](https://crawlee.dev/python/docs/quick-start) `v0.6.0` or higher
...
I had a [Crawlee for Python](https://crawlee.dev/docs/introduction) script pulling article titles from a blog. Simple job — grab every `h2`, dump them to JSON, done. It ran perfectly for days. Then one morning I checked the output file: zero articles. No errors in the logs either. The scraper ran, finished, reported success, and collected absolutely nothing.
...
I built this in Python using [Crawlee](https://crawlee.dev/docs/introduction) and a free LLM endpoint. Here's how.
...
**Stack:**
- [Crawlee for Python](https://crawlee.dev/docs/introduction) (ParselCrawler) — scraping and DOM parsing throughout
...
> **Note:** Before going ahead, if you like this blog, we would be really happy if you gave [Crawlee for Python a star on GitHub](https://github.com/apify/crawlee-python/)! It helps us spread the word to fellow scraper developers.

---

### Step 1: The Original Scraper

I started with a basic [Crawlee ParselCrawler](https://crawlee.dev/python/api/class/ParselCrawler) to scrape article titles from a local test site. You can also scaffold a Crawlee project with `pipx run crawlee['cli'] create my-scraper` — I started from a blank script here to keep things minimal.
...
**Lesson:** Start with the simplest thing that works. A [ParselCrawler](https://crawlee.dev/python/api/class/ParselCrawler) with a hardcoded CSS selector is about 30 lines of code. That's the baseline you're protecting.
...
You can grab a free API key from [OpenRouter](https://openrouter.ai/) — the model I used (`stepfun/step-3.5-flash:free`) doesn't cost anything.
...
**Complex layouts.** My test site has clean, semantic HTML. Real-world sites have deeply nested `<div>` soups, dynamically injected content, shadow DOM, and obfuscated class names. The LLM handles simple structural changes well. It's less reliable when the entire page layout gets redesigned. If you're dealing with fingerprinting-heavy sites, consider using [Camoufox](https://www.crawlee.dev/python/docs/examples/playwright-crawler-with-camoufox) — a browser configuration that's more resistant to detection.

**JavaScript-heavy SPAs.** The `ParselCrawler` fetches raw HTML — if the titles are rendered client-side by JavaScript, they won't be in the response. The fix is straightforward: swap to [Crawlee's PlaywrightCrawler](https://crawlee.dev/python/api/class/PlaywrightCrawler). The self-healing logic works the same way — you'd just pull the rendered DOM from `context.page.content()` instead of `context.http_response`. Check the [Crawlee guide on scraping dynamic websites](https://www.crawlee.dev/blog/scraping-dynamic-websites-using-python) for more on this.
...
**Proxy rotation.** If you're running self-healing scrapers at scale, you'll want to add [proxy management](https://www.crawlee.dev/python/docs/guides/proxy-management#proxy-configuration) to avoid IP-based blocks. The healing logic doesn't help if your requests are being blocked before reaching the page. [Apify Proxy](https://apify.com/proxy) integrates directly with Crawlee via [`ProxyConfiguration`](https://www.crawlee.dev/python/api/class/ProxyConfiguration).
...
The solution I found was the [Universal Bypasser](https://apify.com/macheta/universal-bypasser) Actor on the Apify Store. It uses sophisticated browser profiles and behavioral patterns to get past challenge-response systems and returns clean HTML.
...
The natural next step is running this in the cloud. The Apify platform lets you package any Crawlee scraper as an [Actor](https://docs.apify.com/platform/actors) — a serverless microapp with configurable inputs, built-in storage, and scheduling.
...
With this deployed, you'd [schedule the Actor](https://docs.apify.com/platform/schedules) to run hourly or daily. Every run checks whether the selectors still work. If they break, the Actor heals them and pushes results to the [Dataset](https://docs.apify.com/platform/storage/dataset). You can set up [integrations](https://docs.apify.com/platform/integrations) to pipe the data to Google Sheets, Slack, or a webhook — so you get notified when a repair happens.

To deploy, you'd use the [Apify CLI](https://docs.apify.com/cli/):
...
[Crawlee](https://crawlee.dev/docs/introduction) gave me the scraping foundation — the [request handling](https://www.crawlee.dev/python/docs/guides/request-storage), the parsing, the storage. The LLM gave me the recovery. Together, the scraper runs itself.

---

*Full source code is available on [GitHub](https://github.com/YOUR_USERNAME/self-healing-scraper). Built using [Crawlee for Python](https://crawlee.dev/docs/introduction), [Parsel](https://parsel.readthedocs.io/), and [OpenRouter](https://openrouter.ai/). The test site is included in the repo under `/scripts/test-site` — run `npm run dev` and break things yourself.*
...
*Have questions or want to discuss the implementation? Join the [Apify Discord community](https://discord.com/invite/jyEM2PRvMU) — our community of 11,000+ developers is there to help.*
